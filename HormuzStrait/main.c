...
int main() 
{ 
    const char* filename = "straitRGB.png"; 
    unsigned int width, height;
    int size;
    int bw_size;
    
    // Прочитали картинку
    unsigned char* picture = load_png("straitRGB.png", &width, &height); 
    if (picture == NULL)
    { 
    ...
    // Преобразовываем RGBA в grayscale
    ...
    write_png("bw_pic", bw_pic, width, height);
